...
class ExchangeRates {
  ...
  var cacheGRS = Map.empty[String, Double]
  ...
  // In case of failure repeatedly try to fetch rates with increasing delays between tries
  val fetchGRS = retry(obsOnIO.map(_ => reloadData), pickInc, 0 to 1000 by 10).repeatWhen(_ delay 60.minutes)

  fetchGRS.subscribe { freshFiatRates =>
    cacheGRS = freshFiatRates.toMap
  }
}
